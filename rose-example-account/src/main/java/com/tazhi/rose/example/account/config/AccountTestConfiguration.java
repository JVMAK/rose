package com.tazhi.rose.example.account.config;
...
public class AccountTestConfiguration {
	...
	@Bean 
	public SnapshotService snapshotService(JdbcTemplate jdbcTemplate) {
		return new JdbcSnapshotService(jdbcTemplate);
	}
	
	@Bean
	public EventBus localEventBus() {
		LocalEventBus eventBus = new LocalEventBus();
		eventBus.subscribe(Account.class.getName(), "test-group", e -> {
			System.out.println("EventBus Event: " + e.getClass().getName() + JsonUtils.toJson(e));
